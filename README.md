...
## Overview
This project provides a flexible and extensible framework for conducting rigorous empirical studies on automated unit test generation. The primary goal is to systematically evaluate and compare the effectiveness, cost, and maintainability of tests generated by Large Language Models (LLMs) versus traditional automated test generation tools across benchmarks of varying complexity.

While the feasibility of using LLMs for simple, algorithmic code generation is becoming established , their reliability and performance on complex, real-world enterprise applications remain a significant research gap. This framework is designed to investigate this gap by implementing a hybrid evaluation strategy:

Algorithmic Baseline (HumanEval-X): The framework first evaluates multiple advanced LLM prompting techniques (e.g., Chain-of-Thought, Role-Playing) on the HumanEval-X benchmark. This phase establishes the baseline capabilities of LLMs on simple, self-contained programming tasks, allowing for a controlled comparison of different prompt engineering strategies.

Real-World Application (Spring PetClinic): The best-performing LLM techniques are then benchmarked against traditional tools (EvoSuite, Randoop) on the Spring PetClinic project, a complex, real-world application. This phase tests the tools' ability to handle framework dependencies, mocking, and the nuances of enterprise code.

A novel contribution of this framework is its deep focus on test maintainability. In addition to standard effectiveness metrics like code coverage, the evaluation pipeline uses static analysis (PMD) to automatically measure the structural quality (e.g., Cyclomatic Complexity) of the generated tests. For LLM-generated tests that fail to compile, the framework also quantifies a novel "Correction Distance" metric (using Levenshtein distance) to measure the effort required to make the test usable.

The core architecture is a fully automated, "plug-and-play" system that facilitates the easy integration of various test generators and evaluation metrics, making it a robust platform for reproducible research in AI-driven software engineering.
