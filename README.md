...
While the feasibility of using LLMs for simple, algorithmic code generation is becoming established , their reliability and performance on complex, real-world enterprise applications remain a significant research gap. This framework is designed to investigate this gap by implementing a 
...
A novel contribution of this framework is its deep focus on test maintainability. In addition to standard effectiveness metrics like code coverage, the evaluation pipeline uses static analysis (PMD) to automatically measure the structural quality (e.g., 

Cyclomatic Complexity) of the generated tests. For LLM-generated tests that fail to compile, the framework also quantifies a novel "Correction Distance" metric (using Levenshtein distance) to measure the effort required to make the test usable.
